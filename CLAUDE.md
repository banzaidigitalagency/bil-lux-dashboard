...
- **Export bilan** (voir `docs/memoire/export-bilan.md`) : deux boutons dans le header, xlsx multi-onglets (SheetJS charge a la demande depuis jsdelivr) et csv. Une 8e RPC, `get_dashboard_export(p_client_code, p_scope)`, appelee **seulement au clic** (pas dans `loadDashboard`), renvoie une ligne par jour x annonce (par groupe d'annonces quand la plateforme ne remonte pas le niveau annonce, cas du DOOH) plus une ligne "Non ventile" par jour quand le detail ne couvre pas le total campagne. **Le fichier est telecharge par le client : tout y est en budget client, jamais d'investi media reel.** Cote JS, `enrichExportRows()` deduit creas / variantes / formats / ciblages du nommage (`CREA_CONCEPTS`, `FORMAT_TOKENS`, `TARGET_TOKENS`), `exportRowsCache` est vide par `resetDashboardState()` a chaque changement de campagne.
